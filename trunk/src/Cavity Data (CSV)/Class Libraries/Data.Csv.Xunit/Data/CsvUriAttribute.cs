namespace Cavity.Data
{
    ...
    {
        public CsvUriAttribute(params string[] locations)
            : this()
        {
            if (null == locations)
            {
        ...
        private CsvUriAttribute()
        {
        }

        public IEnumerable<string> Locations { get; private set; }

        public static CsvFile Download(AbsoluteUri location)
        {
            if (null == location)
            {
                                                      ...
                                                      Type[] parameterTypes)
        {
            ...
            else
            {
                ...
                {
                    throw new InvalidOperationException(StringExtensionMethods.FormatWith(Resources.CsvFileAttribute_CountsDiffer, IEnumerableExtensionMethods.Count(Locations), parameterTypes.Length));
                }
#else
                if (Locations.Count() != parameterTypes.Length)
                {
                    throw new InvalidOperationException(Resources.CsvFileAttribute_CountsDiffer.FormatWith(Locations.Count(), parameterTypes.Length));
                }
#endif
